...
sprintf(opus_filename,"%s_%d.mp4",pack_name,p/12);
...
sprintf(repack_cmd,"ffmpeg -i \"%s\" -acodec libopus -ar 48000 -ac 1 -b:a 65k -map_metadata -1 -y %s",out_filename,opus_filename);
printf("command: \"%s\"\n",repack_cmd);
system(repack_cmd);
unlink(out_filename);
}
